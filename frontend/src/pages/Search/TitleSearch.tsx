...
export default function TitleSearch() {
    ...
    // searchData 예시:
    // {
    //   numOfTeams: number; // 전체 게시글 수
    //   teams: SimpleTeamResponse[]; // 현재 페이지의 팀 목록
    // }
    const {
        data: searchData,
        isLoading,
        ...
        queryKey: ['titleSearch', searchKeyword, currentPage],
        queryFn: () => TeamTitleSearch(searchKeyword, String(currentPage)),
        enabled: !!searchKeyword,
    });

    // 한 페이지당 10개씩 → 전체 페이지 수 계산
    // searchData.numOfTeams가 전체 게시글 수
    const totalItems = searchData?.numOfTeams ?? 0;
