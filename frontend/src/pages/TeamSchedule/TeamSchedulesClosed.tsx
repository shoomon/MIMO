import { getClosedSchedules } from '@/apis/TeamAPI';
...
const TeamSchedulesClosed = () => {
    ...
    const { data: sceduleData } = useQuery<TeamSchedulesResponse>({
        queryKey: ['closedScheduleList', teamId],
        queryFn: () => {
            if (!teamId) {
                throw new Error('teamBoardId is undefined');
            }
            return getClosedSchedules(Number(teamId));
        },
...
export default TeamSchedulesClosed;
