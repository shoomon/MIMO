import { getRegularSchedules } from '@/apis/TeamAPI';
...
const TeamSchedulesRegular = () => {
    ...
    const { data: sceduleData } = useQuery<TeamSchedulesResponse>({
        queryKey: ['regularScheduleList', teamId],
        queryFn: () => {
            if (!teamId) {
                throw new Error('teamBoardId is undefined');
            }
            return getRegularSchedules(Number(teamId));
        },
...
export default TeamSchedulesRegular;
